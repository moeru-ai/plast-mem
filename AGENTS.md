...
- Is it about surprise/boundary types? → Start with `crates/core/src/memory/boundary.rs` and `crates/ai/src/lib.rs`
...
- **plastmem_core**: Core structure, including Message, Memory (Episodic retrieval with FSRS + boundary boost re-ranking), and BoundaryType
...
- **Memory creation**: `src/api/add_message.rs` → `MessageQueue::push` → `EventSegmentationJob` → LLM `segment_events` (structured output: summary + surprise + boundary_type) → `EpisodicMemory` with FSRS stability boost
- **Memory retrieval**: `src/api/retrieve_memory.rs` → `EpisodicMemory::retrieve` (BM25 + vector RRF × FSRS retrievability × boundary boost)
